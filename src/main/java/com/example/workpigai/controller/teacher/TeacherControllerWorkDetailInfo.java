...
public class TeacherControllerWorkDetailInfo {
    ...
    public Result deleteWorkDetail(@RequestBody WorkDetail workDetailId) throws Exception {
        //因为前端只是传了一个 id (序号) 过来
        //所以要再通过 id 查询 Class 的其他信息
        WorkDetail workDetail = workDetailService.findById(workDetailId.getId());
        if (workDetail != null){
            workDetailService.deleteById(workDetailId.getId());
            //   删除成功返回码 100
